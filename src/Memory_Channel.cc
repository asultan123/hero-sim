...
template <typename DataType>
void MemoryChannel<DataType>::mem_write_data(const sc_vector<sc_signal<DataType>>& _data)
{
...
template <typename DataType>
const DataType& MemoryChannel<DataType>::channel_read_data_element(unsigned int col)
{
...
template <typename DataType>
void MemoryChannel<DataType>::channel_write_data_element(DataType _data, unsigned int col)
{
    assert(col <= channel_width && col >= 0);
...
template <typename DataType>
unsigned int MemoryChannel<DataType>::addr()
{
    return channel_addr;
}

template <typename DataType>
void MemoryChannel<DataType>::set_addr(unsigned int addr)
{
    channel_addr = addr;
}

template <typename DataType>
void MemoryChannel<DataType>::set_enable(bool status)
{
    channel_enabled = status;
}

template <typename DataType>
bool MemoryChannel<DataType>::enabled()
{
...
template <typename DataType>
void MemoryChannel<DataType>::reset()
{
...
template <typename DataType>
const unsigned int& MemoryChannel<DataType>::get_width()
{
    return channel_width;
}

template <typename DataType>
void MemoryChannel<DataType>::register_port(sc_port_base& port_,
