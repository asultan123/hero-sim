...
template <typename DataType> struct DUMMY_SUB_MODULE_OUT : public sc_module
{
    sc_out<DataType> dummy_sub_module_out;

    DUMMY_SUB_MODULE_OUT(sc_module_name name)
        : sc_module(name), dummy_sub_module_out((string(name) + "dummy_sub_module_out").c_str())
    {
        cout << "DUMMY_SUB_MODULE_OUT MODULE: " << name << " has been instantiated " << endl;
    }
};

template <typename DataType> struct DUMMY_SUB_MODULE_IN : public sc_module
{
    sc_in<DataType> dummy_sub_module_in;

    DUMMY_SUB_MODULE_IN(sc_module_name name)
        : sc_module(name), dummy_sub_module_in((string(name) + "dummy_sub_module_in").c_str())
    {
        cout << "DUMMY_SUB_MODULE_IN MODULE: " << name << " has been instantiated " << endl;
    }
};

template <typename DataType> struct DUMMY_MODULE : public sc_module
{
    Connector connector;
    DUMMY_SUB_MODULE_OUT<DataType> sub_out{"sub_out"};
    DUMMY_SUB_MODULE_IN<DataType> sub_in{"sub_in"};
    // Constructor
    DUMMY_MODULE(sc_module_name name, sc_trace_file *tf)
        : sc_module(name), connector((string(name) + "_connector").c_str(), tf)
    {
...
template <typename DataType> struct Connector_TB : public sc_module
{
