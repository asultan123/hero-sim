...
template <typename DataType> struct Memory_TB : public sc_module
{
    ...
    Memory_TB(sc_module_name name)
        : sc_module(name), tf(sc_create_vcd_trace_file("Prog_trace")),
          control("global_control_channel", sc_time(1, SC_NS), tf), rchannel("read_channel", ram_width, tf),
          wchannel("write_channel", ram_width, tf), payload("payload", ram_width),
          mem("sram", control, channel_count, ram_length, ram_width, tf)
    {
